...
@section('content')
  <div class="min-h-screen w-full flex items-center justify-center">
    <div class="hidden w-full lg:flex lg:w-8/12 flex justify-center">
      <svg class="w-8/12" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 800.012 793.179" role="img" artist="Katerina Limpitsouni" source="https://undraw.co/">
        <g transform="translate(-571.203 -218.417)">
          ...
          <circle cx="24.422" cy="24.422" r="24.422" transform="translate(1050.098 564.909)" class="fill-primary"/>
          ...
          <g transform="translate(1075.599 381.07)">
            ...
            <path d="M523.778,220.925a23,23,0,0,1-.621,5.294l-14.431,78a19.759,19.759,0,0,1-23.787,14.714l-70.488-19.6,3.158-30.523h0l53.679,7.368,6.694-57.1a22.937,22.937,0,0,1,45.8,1.842Z" transform="translate(-376.371 -92.478)" class="fill-primary"/>
            ...
            <rect width="21.152" height="21.152" transform="translate(71.799 589.151)" fill="#ed9da0"/><path d="M469.188,615.868H424.225a25.933,25.933,0,0,1-11.168-2.545l-8.289-3.984a4.978,4.978,0,0,1,.829-9.284l17.1-4.732,29.255-15.377.022.111c.547,2.771,1.395,6.957,1.5,7.211,3,3.437,6.082,5.021,9.16,4.708,5.369-.544,8.938-6.744,8.974-6.807l.027-.048.055.006a3.561,3.561,0,0,1,2.764,1.507c1.225,1.847.478,4.691.355,5.12,1.467,1.435,2.125,9.45,2.176,10.1,2.121,2.248,3.129,4.409,3,6.424a6.918,6.918,0,0,1-2.9,4.861,12.557,12.557,0,0,1-7.893,2.733Z" transform="translate(-379.847 13.676)" fill="#2f2e43"/><path d="M550.486,317.714H449.111L432.951,654.023h35.7l23.066-152.868,92.559,122.294,33.792-30.574-67.584-91.721Z" transform="translate(-371.229 -59.214)" fill="#2f2e43"/>
            ...
            <path d="M518.46,184.739l-41.541,14.442L437.647,401.925l80.814,9.867h39.5l-30.18-201.615a67.286,67.286,0,0,0-9.323-25.438h0Z" transform="translate(-369.923 -96.176)" class="fill-primary"/>
    ...
    <div class="w-full lg:w-4/12 flex flex-col items-center justify-between h-screen p-12">
      <div class="flex items-center justify-between w-full">
        <div class="flex items-center space-x-2">
          <img src="/logo.png" alt="Logo Kode-E" class="w-16">
          <p>Kode-E</p>
        </div>
      ...
      <div class="w-full flex grow flex-col items-center justify-center mt-5">
        @if (session('status'))
          <div role="alert" class="alert alert-success mb-8">
            <span class="iconify lucide--circle-check size-5.5 shrink-0"></span>
            <span>{{ session('status') }}</span>
          </div>
        @endif
        <h1 class="text-3xl font-bold">Login</h1>
        {{-- <p class="text-sm text-center mt-3">Lorem ipsum dolor sit amet consectetur adipisicing elit. Eum, quae?</p> --}}
        <form action="{{ route('login') }}" method="POST" class="w-full mt-5">
          @csrf
          ...
          <fieldset class="fieldset input-password mt-3">
            ...
            @error("password")
              <span class="text-xs text-red-500">{{ $message }}</span>
            @enderror
            <a href="{{ route('password.request') }}" class="w-full text-xs text-end text-primary">Forgot Password?</a>
          </fieldset>
          <button type="submit" class="btn w-full mt-5 bg-primary hover:bg-primary-900 text-white text-sm space-x-2 rounded-lg">
            <i class="fa-solid fa-right-to-bracket"></i>
            <span>Login</span>
          </button>
          <a href="#" class="btn w-full mt-5 bg-white hover:bg-gray-200 text-sm font-normal space-x-2 rounded-lg">
            <i class="fa-brands fa-google"></i>
            <span>Login with Google</span>
          </a>
        </form>
        <p class="text-center mt-5 font-normal text-sm">
          Haven't account?
